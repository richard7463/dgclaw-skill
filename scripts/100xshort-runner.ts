...
const STATE_PATH = join(STATE_DIR, '100xshort-runner-state.json');
const HL_API_URL = 'https://api.hyperliquid.xyz';
const transport = new HttpTransport({ url: HL_API_URL });

const PAIR = 'ETH';
const POLL_SECONDS = parseInt(process.env.MIRAIX_100X_SHORT_POLL_SECONDS ?? '30', 10);
const MIN_NOTIONAL = parseFloat(process.env.MIRAIX_100X_SHORT_MIN_NOTIONAL ?? '12');
const MAX_MARGIN_USDC = parseFloat(process.env.MIRAIX_100X_SHORT_MAX_MARGIN_USDC ?? '8');
const MARGIN_FRACTION = parseFloat(process.env.MIRAIX_100X_SHORT_MARGIN_FRACTION ?? '0.2');
const MAX_NOTIONAL_USDC = parseFloat(process.env.MIRAIX_100X_SHORT_MAX_NOTIONAL_USDC ?? '800');
const BREAKDOWN_LOOKBACK = parseInt(process.env.MIRAIX_100X_SHORT_BREAKDOWN_LOOKBACK ?? '20', 10);
const VOLUME_MULTIPLIER = parseFloat(process.env.MIRAIX_100X_SHORT_VOLUME_MULTIPLIER ?? '1.5');
const RANGE_MULTIPLIER = parseFloat(process.env.MIRAIX_100X_SHORT_RANGE_MULTIPLIER ?? '1.3');
const BREAK_EVEN_ROE = parseFloat(process.env.MIRAIX_100X_SHORT_BREAK_EVEN_ROE ?? '0.0022');
const STOP_LOSS_MULTIPLIER = parseFloat(process.env.MIRAIX_100X_SHORT_STOP_LOSS_MULTIPLIER ?? '1.0018');
const TAKE_PROFIT_MULTIPLIER = parseFloat(process.env.MIRAIX_100X_SHORT_TAKE_PROFIT_MULTIPLIER ?? '0.996');
const TIMEOUT_MS = parseInt(process.env.MIRAIX_100X_SHORT_TIMEOUT_MS ?? String(8 * 60 * 1000), 10);
const PAUSE_HOURS = parseInt(process.env.MIRAIX_100X_SHORT_LOSS_PAUSE_HOURS ?? '12', 10);
const DAILY_LOSS_LIMIT = parseFloat(process.env.MIRAIX_100X_SHORT_DAILY_LOSS_LIMIT ?? '0.06');
...
async function postSignal(title: string, content: string) {
  ...
  try {
    ...
    console.error(`[100xshort] failed to post signal: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
async function closePosition(exchange: ExchangeClient, info: InfoClient, user: `0x${string}`, assetId: number) {
  ...
  const result = await exchange.order({
    orders: [{ a: assetId, b: true, r: true, p: orderPrice, s: size, t: { limit: { tif: 'Ioc' } } }],
    grouping: 'na',
  });
...
async function buildSignal(info: InfoClient) {
  ...
  if (candles5m.length < 60 || candles1m.length < BREAKDOWN_LOOKBACK + 25) {
  ...
  const history = candles1m.slice(-(BREAKDOWN_LOOKBACK + 21), -1);
  const breakdownWindow = history.slice(-BREAKDOWN_LOOKBACK);
  const recent20 = history.slice(-20);
  const breakdownLow = Math.min(...breakdownWindow.map((c) => parseFloat(c.l)));
  ...
  if (close >= breakdownLow) return { ok: false as const, reason: 'breakdown not triggered', breakdownLow, close };
...
async function processClosedTrade(info: InfoClient, state: RunnerState, user: `0x${string}`) {
  ...
  await postSignal(
    `Closed ${openTrade.pair} short (${reason})`,
    `Closed ${openTrade.pair} short. Entry ${openTrade.entryPx.toFixed(2)}. Size ${openTrade.size}. Leverage ${openTrade.leverage}x. Realized PnL ${realizedPnl.toFixed(4)} USDC. Reason: ${reason}.`,
  );
...
async function evaluateOnce() {
  ...
  if (position) {
    ...
    if (!state.openTrade) {
      ...
      if (existingOrders.length < 2) {
        ...
        console.log(`[100xshort] adopted existing ${PAIR} short and attached brackets.`);
      } else {
        console.log(`[100xshort] adopted existing ${PAIR} short with ${existingOrders.length} existing reduce-only orders.`);
      }
      saveState(state);
      return;
    }
    ...
    if (!openTrade.stopMovedToBreakeven && roe >= BREAK_EVEN_ROE) {
      ...
      await postSignal(
        `Moved ${PAIR} short stop to breakeven`,
        `Trade moved in favor. ${PAIR} short stop loss raised to breakeven at ${openTrade.entryPx.toFixed(2)}. Leverage ${openTrade.leverage}x.`,
      );
    }
    ...
    if (Date.now() - openTrade.openedAt >= TIMEOUT_MS) {
      ...
      await postSignal(
        `Closed ${PAIR} short (timeout)`,
        `Closed ${PAIR} short after timeout. Entry ${openTrade.entryPx.toFixed(2)}. Size ${openTrade.size}. Leverage ${openTrade.leverage}x.`,
      );
  ...
  const result = await exchange.order({
    orders: [{ a: assetId, b: false, r: false, p: orderPrice, s: size, t: { limit: { tif: 'Ioc' } } }],
    grouping: 'na',
  });
  ...
  state.openTrade = {
    ...
    takeProfit,
    stopMovedToBreakeven: false,
    signalLow: signal.breakdownLow,
    notional,
  };

  await postSignal(
    `Opened ${PAIR} short`,
    `Opened ${PAIR} short. Entry ${entryPx.toFixed(2)}. Size ${filledSize}. Leverage target ${leverageTarget}x, actual ${actualLeverage}x. Notional ${notional.toFixed(2)} USD. TP ${takeProfit.toFixed(2)}. SL ${stopLoss.toFixed(2)}.`,
  );
...
async function main() {
  ...
  if (mode === 'daemon') {
    console.log(`100XSHORT runner started in daemon mode. Polling every ${POLL_SECONDS}s.`);
    while (true) {
      try {
        await evaluateOnce();
      } catch (error) {
        console.error(`[100xshort] ${error instanceof Error ? error.stack ?? error.message : String(error)}`);
      }
